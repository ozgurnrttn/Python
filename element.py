element_kütleleri = {"D": 2.014102, "T": 3.016049,
...
while True:
    ...
    elementler = molekül_formülü.split(" ")
    hesaplama = {}
    atom_sayısı = {}
    for i in elementler:
        if i[:2] in element_kütleleri.keys():
            hesaplama[i[:2]] = element_kütleleri[i[:2]]
            if i[2:] == '':
                atom_sayısı[i[:2]] = 1
            else:
                atom_sayısı[i[:2]] = i[2:]
        ...
        elif i[:1] in element_kütleleri.keys():
            hesaplama[i[:1]] = element_kütleleri[i[:1]]
            if i[1:] == '':
                atom_sayısı[i[:1]] = 1
            else:
                atom_sayısı[i[:1]] = i[1:]
            
    #print("Hesaplama: ",hesaplama)
    #print("Atom sayısı: ",atom_sayısı)
    #hesaplama["C"] * atom_sayısı["C"]
    molekül_ağırlığı = 0
    ...
    #print("Yüzdeler: ",yüzdeler)
    for k in yüzdeler.keys():
        print("Element: %s, "%k + "yüzdesi: %",round(yüzdeler[k],2))
    sorgu = input("Devam etmek için E, programı sonlandırmak için H tuşuna basınız: ")
